...
function StatCard({ stat, index }: { stat: typeof LANDING_STATS[number]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="text-center landing-glass rounded-[20px] p-6 shadow-sm hover:shadow-md transition-all"
    >
      <div className="font-mono-numbers text-3xl sm:text-4xl lg:text-5xl font-bold text-primary mb-2">
        {isInView ? (
          <CountUp
            end={stat.value}
            duration={2.5}
